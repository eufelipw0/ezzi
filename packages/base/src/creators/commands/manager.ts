...
type StoredAppCommandData =
    ...
    & { modules?: CommandModule[]; category?: string };
...
function resolveOptionValueType(type: ApplicationCommandOptionType): OptionValueType {
    switch (type) {
...
function resolveMissingPermissions(
    ...
    return required.filter((perm) => !member.permissions.has(perm as Parameters<PermissionsBitField["has"]>[0]));
}

export class CommandManager {

    constructor(private app: LithiumApp) {}
    ...
    private readonly aliasCollection = new Collection<string, StoredAppCommandData>();
    private readonly commandRunners = new Collection<string, Runner[]>();
    private readonly autocompleteRunners = new Collection<string, AutocompleteRun<string | number, any>>();
    private readonly optionDefs = new Collection<string, SlashCommandPrimitiveOptionData<boolean>[]>();
    private readonly shortcuts = new Collection<string, string>();
    private readonly botPermissionsMap = new Collection<string, PermissionResolvable[]>();
    public readonly logs: string[] = [];

    private formatName(name: string, type = ApplicationCommandType.ChatInput): string {
        return limitText(
    ...
    public getAutocompleteHandler(...path: (string | null)[]): AutocompleteRun<string | number, any> | undefined {
    ...
    public getHandler(type: ApplicationCommandType, ...path: (string | null)[]): Runner[] | undefined {
        const commandName = path[0];
        const resolved = `/${type}/${path.filter(isDefined).join("/")}`;
        return (
    ...
    ): { runners: Runner[]; optionDefs: SlashCommandPrimitiveOptionData<boolean>[]; botPermissions?: PermissionResolvable[] } | undefined {
        const type = ApplicationCommandType.ChatInput;
    ...
    private buildOptions(options: SlashCommandOptionData<boolean>[], path: string): ApplicationCommandOptionData[] {
        ...
        for (const option of options) {
            ...
            switch (option.type) {
                case ApplicationCommandOptionType.SubcommandGroup: {
                    const { options: subcommands, defaultMemberPermissions, botPermissions, ...data } = option;
                ...
                case ApplicationCommandOptionType.Subcommand: {
                    const { options: subOpts, defaultMemberPermissions, botPermissions, ...data } = option;
                    if (botPermissions?.length) {
                        this.botPermissionsMap.set(`${path}/${data.name}`, botPermissions);
                    }
                    resolved.push({
    ...
    private resolveModules(
        ...
        for (const group of groups) {
            ...
            const slashSubs = groupSubs.filter((s) => (s.ignore as unknown) !== IgnoreCommand.Slash);
            ...
            for (const sub of groupSubs) {
                ...
                if (sub.options?.length) {
                    this.optionDefs.set(subPath, sub.options as SlashCommandPrimitiveOptionData<boolean>[]);
                }
    ...
    public set(data: GenericAppCommandData): StoredAppCommandData {
        ...
        const commandData: StoredAppCommandData = { ...data, name, type, dmPermission };
        ...
        if (data.options?.length) {
            const primitive = (data.options as SlashCommandOptionData<boolean>[]).filter(
                (o) =>
    ...
    public build(): BuildedCommandData[] {
        const result = Array.from(this.collection.values())
            ...
            .map((raw) => {
                const { options, modules, description, descriptionLocalizations, category, botPermissions: _bp, ...data } = raw;
    ...
    public async onPrefixCommand(message: Message): Promise<void> {
        ...
        const { middleware, onMemberPermissionsFailed, onBotPermissionsFailed, onOptionsError, onError } = this.config;
        ...
        if (onMemberPermissionsFailed && rawCommandData?.defaultMemberPermissions) {
            const perms = Array.isArray(rawCommandData.defaultMemberPermissions)
                ...
                : [rawCommandData.defaultMemberPermissions as PermissionResolvable];
            const missing = resolveMissingPermissions(message.member as GuildMember | null, perms);
            if (missing.length) {
        ...
        if (onOptionsError && optionDefs.length) {
            const missing: OptionNeed[] = optionDefs
                .filter((def) => def.required && args[optionDefs.indexOf(def)] === undefined)
                .map((def) => ({
                    name: def.name,
    ...
    public async onCommand(interaction: CommandInteraction): Promise<void> {
        ...
        const { onNotFound, middleware, onError, onMemberPermissionsFailed, onBotPermissionsFailed } = this.config;
    ...
    public async register(client: Client<true>): Promise<void> {
        ...
        const createVerboseLogs = (cmdsCollection: Collection<string, ApplicationCommand>) =>
            cmdsCollection.map(({ id, name, type, client: cl, createdAt, guild }) => {
                const [icon] = this.getTitle({ type: type as CommandType } as GenericAppCommandData);
                return ck.dim.green(
                    spaceBuilder(
                        ` └ ${icon}`,
        ...
        const logRegistration = (cmdsCollection: Collection<string, ApplicationCommand>, location: string) => {
